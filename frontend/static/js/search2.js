document.addEventListener("DOMContentLoaded", () => {
    flight_duration();
    filter_price();
    const clrBtn = document.querySelector(".clr-filter-div button");
    console.log("[DEBUG] .clr-filter-div button:", clrBtn);
    if (!clrBtn) {
        console.log("[DEBUG] .clr-filter-div:", document.querySelector(".clr-filter-div"));
        console.log("[DEBUG] All buttons:", document.querySelectorAll("button"));
        console.log("[DEBUG] Document body:", document.body.innerHTML);
    }
    if (clrBtn) {
        clrBtn.addEventListener('click', reset_filter);
    }
    ...
    if (tripIdentifier && tripIdentifier.value === '2') {
        flight_duration2();
        filter_price2();
        const clrBtn2 = document.querySelector(".clr-filter-div2 button");
        console.log("[DEBUG] .clr-filter-div2 button:", clrBtn2);
        if (!clrBtn2) {
            console.log("[DEBUG] .clr-filter-div2:", document.querySelector(".clr-filter-div2"));
        }
        if (clrBtn2) {
            clrBtn2.addEventListener('click', reset_filter2);
        }
    }
});
...
function filter_price() {
    let rangeInput = document.querySelector(".filter-price input[type=range]");
    let priceValue = document.querySelector(".filter-price .final-price-value");
    if (!rangeInput || !priceValue) return;
    ...
    for (let i = 0; i < flights.length; i++) {
        ...
        if (parseInt(priceSpan.innerText) > parseInt(value)) {
            flights[i].classList.add('hide');
            flights[i].classList.remove('show');
        }
        else {
